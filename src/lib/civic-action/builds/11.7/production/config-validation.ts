/**
 * CAE-11.7-W8 — Configuration and environment validation
 */
...
export function validateCommunicationProductionConfiguration(): ReadinessCheck[] {
  ...
  if (existsSync(manifestPath)) {
    ...
    checks.push({
      check_id: "com-feature-flags",
      category: "communication_flags",
      title: "Communication engine feature flags",
      status: manifest.communication_feature_flags?.COMMUNICATION_ENGINE_ENABLED ? "pass" : "fail",
      detail: "COMMUNICATION_ENGINE_ENABLED must be true for launch",
      blocking: true,
    });
    checks.push({
...
export function communicationConfigurationReady(): boolean {
  return validateCommunicationProductionConfiguration().filter((c) => c.blocking).every((c) => c.status === "pass");
}
