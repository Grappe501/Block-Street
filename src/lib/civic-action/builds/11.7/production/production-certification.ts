/**
 * CAE-11.7-W8 — Production certification levels
 */
import type { ProductionCertificationLevel, ProductionCertificationView } from "./contracts";
import { communicationConfigurationReady } from "./config-validation";
import { communicationBootstrapReady } from "./migration-bootstrap";
import { runCommunicationCertificationSuite } from "./certification-suite";
import { communicationOperationsReady } from "./operational-readiness";
...
export function evaluateCommunicationProductionCertifications(
  ...
  const configOk = communicationConfigurationReady();
  const bootstrapOk = communicationBootstrapReady();
  const opsOk = communicationOperationsReady();
  ...
  return levels.map((level) => {
    ...
    if (level === "ready_for_statewide_deployment") {
      ...
      if (!hasCommunicationApprovedSignOff(institutionId, "statewide")) {
        blockers.push("Executive statewide sign-off required");
      } else {
        evidence.push("Executive statewide sign-off recorded");
      }
      if (!hasCommunicationApprovedSignOff(institutionId, "release")) {
...
export function highestCommunicationAchievedLevel(institutionId: string): ProductionCertificationLevel | null {
  const levels = evaluateCommunicationProductionCertifications(institutionId);
