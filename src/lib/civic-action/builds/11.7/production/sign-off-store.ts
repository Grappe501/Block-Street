/**
 * CAE-11.7-W8 — Executive sign-off records (Humans approve launch)
 */
import { caeId, nowIso } from "../../../utils";
import { readStoreSlice, writeStoreSlice } from "../services/repository";
import type { ExecutiveSignOffRecord } from "./contracts";

const KEY = "communication_production_sign_offs";

export function recordCommunicationExecutiveSignOff(input: {
...
export function listCommunicationExecutiveSignOffs(institutionId: string, limit = 50): ExecutiveSignOffRecord[] {
  return readStoreSlice<ExecutiveSignOffRecord>(KEY)
    .filter((s) => s.institution_id === institutionId)
    .slice(-limit);
}

export function hasCommunicationApprovedSignOff(
