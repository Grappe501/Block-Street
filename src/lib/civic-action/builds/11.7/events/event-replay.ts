/**
 * CAE-11.7-W5 — Event replay (facts only; never re-execute commands)
 */
import { readStoreSlice, COMMUNICATION_DOMAIN_EVENTS_KEY } from "../services/repository";
import type { CommunicationDomainEvent, CommunicationEventOutboxRecord } from "../services/events";
import { processOutboxForConsumer } from "./outbox-publisher";

export function listCommunicationDomainEvents(initiativeId?: string, limit = 200) {
...
export function replayCommunicationEvents(input: {
  initiative_id?: string;
  consumer_name: string;
  from_event_id?: string;
  limit?: number;
}) {
  const events = listCommunicationDomainEvents(input.initiative_id, input.limit ?? 100);
  ...
  for (const event of slice) {
    const record: CommunicationEventOutboxRecord = {
