/**
 * CAE-11.2-W5 — Webhook delivery (signed, replay-resistant)
 */
...
export type ObjectiveWebhookSubscription = {
  id: string;
...
const SUBS_KEY = "objective_webhook_subscriptions";
const DELIVERY_KEY = "objective_webhook_deliveries";
const SECRETS_KEY = "objective_webhook_secrets";

export function createWebhookSubscription(input: Omit<ObjectiveWebhookSubscription, "id" | "secret_reference">) {
  ...
  const sub: ObjectiveWebhookSubscription = {
    ...input,
    id: caeId("whk"),
    secret_reference: secretRef,
  };
  const subs = readStoreSlice<ObjectiveWebhookSubscription>(SUBS_KEY);
  subs.push(sub);
...
export function enqueueWebhookDelivery(subscriptionId: string, eventId: string, payload: Record<string, unknown>) {
  ...
  return { delivery, headers: { "X-Objective-Timestamp": timestamp, "X-Objective-Signature": signature } };
}

export function enqueueObjectiveWebhookDeliveries(record: ExecutionEventOutboxRecord) {
  const institutionId = (record.payload.institution_id as string) ?? "";
  const subs = readStoreSlice<ObjectiveWebhookSubscription>(SUBS_KEY).filter(
    (s) => s.institution_id === institutionId && s.status === "active" && s.event_types.includes(record.event_type)
  );
  return subs.map((s) =>
    enqueueWebhookDelivery(s.id, record.event_id, {
