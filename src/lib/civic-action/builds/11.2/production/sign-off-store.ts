/**
 * CAE-11.2-W8 — Executive sign-off records (Humans approve launch)
 */
import { caeId, nowIso } from "../../../utils";
import { readStoreSlice, writeStoreSlice } from "../services/repository";
import type { ExecutiveSignOffRecord } from "./contracts";

const KEY = "objective_production_sign_offs";

export function recordObjectiveExecutiveSignOff(input: {
...
export function listObjectiveExecutiveSignOffs(institutionId: string, limit = 50): ExecutiveSignOffRecord[] {
  return readStoreSlice<ExecutiveSignOffRecord>(KEY)
    .filter((s) => s.institution_id === institutionId)
    .slice(-limit);
}

export function hasObjectiveApprovedSignOff(
