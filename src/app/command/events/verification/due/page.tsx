...
export default function CommandVerificationDuePage() {
  ...
  return (
    <CommandChrome title="Verification due" subtitle="Overdue verification items" backHref="/command/events/verification" backLabel="Verification" nav={<CommandCalendarNav />}>
      <VerificationSoftBetaNote />
      <CommandSection title={`${rows.length} event(s) with overdue verification`}>
        <ul className="space-y-2 font-fieldSans text-sm">
          {rows.map(({ event, summary }) => (
            <li key={event.event_id} className="rounded border bg-white p-3">
              ...
              <p className="text-field-ink/70">{summary.overdueCount} overdue</p>
            </li>
          ))}
        </ul>
      </CommandSection>
