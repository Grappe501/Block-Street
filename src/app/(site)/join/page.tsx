...
export default async function JoinPage({
  ...
  // Step 2a: Student picked county → show schools in that county
  if (path === "student" && countySlug) {
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-12">
        <Link href="/join?path=student" className="text-sm text-brand-600 hover:underline">
          ← Back to counties
        </Link>
        ...
        {schools.length === 0 ? (
          <div className="mt-8 card bg-amber-50 border-amber-200">
            ...
            <Link href={`/county/${countySlug}`} className="mt-4 inline-block btn-primary">
  ...
  // Step 1a: Student path → pick county first
  if (path === "student") {
    return (
      <div className="mx-auto max-w-4xl px-4 py-12">
        <Link href="/join" className="text-sm text-brand-600 hover:underline">← Back</Link>
        ...
        <div className="mt-8 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {counties.map((county) => {
            ...
            return (
              <Link
                key={county.slug}
                href={`/join?path=student&county=${county.slug}`}
  ...
  // County-only path for non-students
  if (path === "county") {
    return (
      <div className="mx-auto max-w-4xl px-4 py-12">
        <Link href="/join" className="text-sm text-brand-600 hover:underline">← Back</Link>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      ...
      <div className="mt-10 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
        <strong>Coming July 14:</strong> Full signup with profile, share link, QR code, and personal network board.
      </div>
    </div>
  );
}
