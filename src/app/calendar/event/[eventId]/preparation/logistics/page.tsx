...
export default async function EventPreparationLogisticsPage({ params }: { params: Promise<{ eventId: string }> }) {
  const { eventId } = await params;
  const event = getEventById(eventId);
  if (!event) notFound();
  ensurePreparationFromEvent(event);
  const rows = listPreparationRows(eventId, "logistics");

  return (
    <CalendarChrome title="Logistics" subtitle={event.title} backHref={`/calendar/event/${eventId}/preparation`} backLabel="Preparation">
      <CalendarHonestyBanner />
      <PreparationSoftBetaNote />
      <EventSubnav eventId={eventId} />
      <CalendarSection title="Logistics checklist">
