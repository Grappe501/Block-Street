...
export default async function EventPreparationRemindersPage({ params }: { params: Promise<{ eventId: string }> }) {
  const { eventId } = await params;
  const event = getEventById(eventId);
  if (!event) notFound();
  ensurePreparationFromEvent(event);
  const rows = listPreparationRows(eventId, "reminder");

  return (
    <CalendarChrome title="Reminders" subtitle={event.title} backHref={`/calendar/event/${eventId}/preparation`} backLabel="Preparation">
      <CalendarHonestyBanner />
      <PreparationSoftBetaNote />
      <EventSubnav eventId={eventId} />
      <CalendarSection title="Reminder schedule">
