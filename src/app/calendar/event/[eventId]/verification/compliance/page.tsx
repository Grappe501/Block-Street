...
export default async function EventVerificationCompliancePage({ params }: { params: Promise<{ eventId: string }> }) {
  const { eventId } = await params;
  const event = getEventById(eventId);
  if (!event) notFound();
  ensureVerificationFromEvent(event);
  const rows = [...listVerificationRows(eventId, "legal"), ...listVerificationRows(eventId, "campus")];

  return (
    <CalendarChrome title="Compliance verification" subtitle={event.title} backHref={`/calendar/event/${eventId}/verification`} backLabel="Verification">
      <CalendarHonestyBanner />
      <VerificationSoftBetaNote />
      <EventSubnav eventId={eventId} />
      <CalendarSection title="Compliance">
