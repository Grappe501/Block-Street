...
export default async function Page({ params }: { params: Promise<{ collegeSlug: string }> }) {
  ...
  return (
    <CollegeChrome slug={college.slug} name={college.name} title="Follow-up scripts">
      <p className="text-sm text-slate-700">After interest / after no response — one clear next step each time.</p>
