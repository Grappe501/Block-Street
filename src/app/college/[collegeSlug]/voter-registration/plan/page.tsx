...
export default async function Page({ params }: { params: Promise<{ collegeSlug: string }> }) {
  ...
  return (
    <CollegeChrome slug={college.slug} name={college.name} title="Plan workflow">
      <p className="text-sm text-slate-700">Confirm campus and legal requirements → dates → locations → permissions → team → materials → volunteers → shifts → promote → conduct → record → follow up.</p>
