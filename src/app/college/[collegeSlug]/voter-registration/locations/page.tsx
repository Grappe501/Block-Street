...
export default async function Page({ params }: { params: Promise<{ collegeSlug: string }> }) {
  ...
  return (
    <CollegeChrome slug={college.slug} name={college.name} title="Locations">
      <p className="text-sm text-slate-700">Identify high-traffic campus places. Permissions are verification gates — not assumed.</p>
