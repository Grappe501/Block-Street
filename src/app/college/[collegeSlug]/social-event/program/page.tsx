...
export default async function Page({ params }: { params: Promise<{ collegeSlug: string }> }) {
  ...
  return (
    <CollegeChrome slug={college.slug} name={college.name} title="Program flow">
      <p className="text-sm text-slate-700">Welcome → short College Community intro → introductions → team showcase → VR invite → role matching → Power of 5 → social → next steps.</p>
