...
export const GET = withApiGateway(
  async (ctx, request) =>
    withCommunicationApi(ctx, request, (apiCtx) => ({
      health: communicationProductionService.getHealthSummary(apiCtx.institution_id),
    })),
  { permission: "civic_action.view", endpoint: "/api/v1/communications/production/health" }
);
