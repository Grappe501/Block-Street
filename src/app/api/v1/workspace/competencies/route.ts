...
export const GET = withApiGateway(
  async (ctx, request) =>
    withKnowledgeApi(ctx, request, (apiCtx) => ({
      competencies: knowledgeExperienceService.getCompetencies(toExperienceContext(apiCtx)),
      event: "competency.viewed",
    })),
  { permission: "training.view", endpoint: "/api/v1/workspace/competencies" }
);
