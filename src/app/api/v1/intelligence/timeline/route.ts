...
export const GET = withApiGateway(
  async (ctx, request) =>
    withInitiativeApi(ctx, request, (apiCtx) => ({
      collisions: initiativeIntelligenceService.getTimelineCollisions(toIntelligenceContext(apiCtx)),
      advisory_only: true,
    })),
  { permission: "civic_action.view", endpoint: "/api/v1/intelligence/timeline" }
);
