...
export const GET = withApiGateway(
  async (ctx, request) =>
    withObjectiveApi(ctx, request, (apiCtx) => {
      ...
      return {
        progress: objectiveIntelligenceService.getProgress(intelCtx),
        execution: objectiveIntelligenceService.getExecution(intelCtx),
        advisory_only: true,
      };
    }),
  { permission: "civic_action.view", endpoint: "/api/v1/intelligence/objectives/progress" }
);
