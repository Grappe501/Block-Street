...
export const GET = withApiGateway(
  async (ctx, request) =>
    withObjectiveApi(ctx, request, (apiCtx) => {
      ...
      return {
        brief: objectiveIntelligenceService.getBriefing(intelCtx),
        patterns: objectiveIntelligenceService.getPatterns(apiCtx.institution_id, initiativeId),
        historical: objectiveIntelligenceService.getHistoricalInsights(apiCtx.institution_id),
        advisory_only: true,
      };
    }),
  { permission: "civic_action.view", endpoint: "/api/v1/intelligence/objectives/executive" }
);
