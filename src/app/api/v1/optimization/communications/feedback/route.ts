...
import type { OptimizationFeedbackAction } from "@/lib/civic-action/builds/11.7/optimization";
...
export const POST = withApiGateway(
  async (ctx, request) =>
    withCommunicationApi(ctx, request, async (apiCtx) => {
      ...
      const entry = communicationOptimizationService.recordFeedback({
  ...
  { permission: "civic_action.view", endpoint: "/api/v1/optimization/communications/feedback" }
);
