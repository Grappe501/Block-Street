...
import type { LaunchControlView, ProductionCertificationView, ReadinessCheck } from "@/lib/civic-action/builds/11.12/production/contracts";
...
export function KnowledgeLaunchControlCenter({
  launch,
  levels,
  gates,
  onSignOff,
  ...
  gates?: { gate_id: string; title: string; status: string }[];
  onSignOff?: (type: "release" | "pilot" | "limited_production" | "general_availability") => void;
}) {
  ...
  return (
    <div className="space-y-8" aria-label="Knowledge launch command center">
      <section className={`rounded-xl border p-6 ${DECISION_STYLES[launch.decision]}`}>
        ...
        <p className="mt-2 text-sm">
          Recommendation: {launch.launch_recommendation.replace(/_/g, " ")} · Checklist {launch.checklist_complete_pct}%
          complete · Rollback available
        </p>
        {launch.critical_issues.length > 0 && (
          <ul className="mt-4 list-disc space-y-1 pl-5 text-sm" role="status">
        ...
        {launch.executive_sign_off_required && onSignOff && (
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => onSignOff("limited_production")}
              className="rounded bg-slate-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-slate-800"
            >
              Record limited production sign-off
            </button>
          </div>
        )}
      </section>
      ...
      {gates && gates.length > 0 && (
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          <h2 className="text-lg font-semibold text-slate-900">Certification gates</h2>
          <ul className="mt-4 space-y-2 text-sm">
            {gates.map((g) => (
              <li key={g.gate_id} className="flex gap-2">
                <span className={`font-medium ${STATUS_STYLES[g.status as ReadinessCheck["status"]] ?? ""}`}>
                  {g.status}
                </span>
                <span>{g.gate_id}</span>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {levels.map((l) => (
            <div
              key={l.level}
              className={`rounded-lg border p-4 ${l.achieved ? "border-green-200 bg-green-50" : "border-slate-200"}`}
            >
              <p className="font-medium text-slate-900">{l.label}</p>
              <p className="mt-1 text-xs text-slate-600">{l.achieved ? "Achieved" : l.blockers[0] ?? "Not yet"}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6">
        ...
        <ChecklistSection title="Configuration" checks={byCategory("configuration").concat(byCategory("feature"))} />
        <ChecklistSection title="Bootstrap" checks={byCategory("bootstrap")} />
        <ChecklistSection title="Operations" checks={byCategory("operations").concat(byCategory("ops"))} />
      </section>
    </div>
  );
}
