...
import type { LaunchControlView, ProductionCertificationView } from "@/lib/civic-action/builds/11.7/production/contracts";
import { CommunicationLaunchControlCenter } from "./CommunicationLaunchControlCenter";

export function CommunicationProductionLaunchDashboard() {
  ...
  const load = useCallback(() => {
    fetch("/api/v1/communications/production/launch")
      .then((r) => r.json())
      .then((d) => setLaunch(d.data?.launch ?? null));
    fetch("/api/v1/communications/production/certification")
      .then((r) => r.json())
      .then((d) => setLevels(d.data?.production_levels ?? []));
    fetch("/api/v1/communications/production/health")
      .then((r) => r.json())
  ...
  async function handleSignOff(type: "release" | "pilot" | "organization" | "statewide") {
    await fetch("/api/v1/communications/production/sign-off", {
      method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      <CommunicationLaunchControlCenter launch={launch} levels={levels} onSignOff={handleSignOff} />
    </div>
  );
}
