...
export function SimulationWorkspaceRemote({ institutionId }: { institutionId: string }) {
  ...
  async function runSim() {
    ...
    setError("");
    try {
      const res = await fetch("/api/v1/optimization/communications/simulate", {
        method: "POST",
      ...
      setResult((data as { simulation?: SimulationResult }).simulation ?? null);
    } catch {
