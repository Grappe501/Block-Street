...
import type { LaunchControlView, ProductionCertificationView } from "@/lib/civic-action/builds/11.1/production/contracts";
import { LaunchControlCenter } from "./LaunchControlCenter";

export function ProductionLaunchDashboard() {
  ...
  const load = useCallback(() => {
    fetch("/api/v1/production/launch")
      .then((r) => r.json())
      .then((d) => setLaunch(d.data?.launch ?? null));
    fetch("/api/v1/production/certification")
      .then((r) => r.json())
      .then((d) => setLevels(d.data?.production_levels ?? []));
    fetch("/api/v1/production/health")
      .then((r) => r.json())
  ...
  async function handleSignOff(type: "release" | "pilot" | "organization" | "statewide") {
    await fetch("/api/v1/production/sign-off", {
      method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      <LaunchControlCenter launch={launch} levels={levels} onSignOff={handleSignOff} />
    </div>
  );
}
