...
const items = cal.ensureRsvpFromEvent(event);
assert(items.length >= 3, `rsvp items seeded (${items.length})`);
assert(items.every((i) => i.softBeta && !i.durableAuthority), "soft beta only");
assert(items.some((i) => i.category === "plan"), "plan");
console.log("test:calendar:rsvp-model PASS");
