...
const items = cal.ensureFollowUpFromEvent(event);
assert(items.length >= 4, `template report items seeded (${items.length})`);
assert(items.every((i) => i.softBeta && !i.durableAuthority), "soft beta only");
assert(items.some((i) => i.category === "metric"), "metrics");
assert(items.some((i) => i.category === "action"), "actions");
console.log("test:calendar:followup-model PASS");
